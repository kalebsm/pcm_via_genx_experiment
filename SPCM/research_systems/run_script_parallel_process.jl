...
function log_process(file, script_name, folder, log)
    try
...
open(log_file, "w") do log
    ...
    @threads for folder in subfolders
        # Get runpath
        ...
        log_process(joinpath(runpath, "Run_spcm_pf.jl"), "Run_spcm_pf.jl", folder, log)
        log_process(joinpath(runpath, "Run_spcm_dlac-p.jl"), "Run_spcm_dlac-p.jl", folder, log)
        log_process(joinpath(runpath, "Run_spcm_dlac-i.jl"), "Run_spcm_dlac-i.jl", folder, log)
        log_process(joinpath(runpath, "Run_spcm_slac.jl"), "Run_spcm_slac.jl", folder, log)
    end
