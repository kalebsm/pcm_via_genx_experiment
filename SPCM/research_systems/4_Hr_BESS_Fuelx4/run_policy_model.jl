...
function run_policy_model(model_type, case)
    ...
    model_type = split(split(basename(@__FILE__), ".jl")[1], "_")[2]
    ...
    norta_path = joinpath("src", "scenario_generation", "sequential_norta")
    ...
    # sP = Dict()
    pgen_dp = zeros(Tend, num_gen);
    # sRSV = Dict()
    rsv_dp = zeros(Tend, num_gen);
    # sREG = Dict()
    reg_dp = zeros(Tend, num_gen);

    # sNSE = Dict()
    nse_dp = zeros(Tend, 1);
    unmet_rsv_dp = zeros(Tend, 1);

    # sCOMMIT = Dict()
    commit_dp = zeros(Tend, num_gen);
    # sSTART = Dict()
    start_dp = zeros(Tend, num_gen);
    # sSHUT = Dict()
    shut_dp = zeros(Tend, num_gen);


    # sCHARGE = Dict()
    charge_dp = zeros(Tend, num_gen);
    # sS = Dict()
    s_dp = zeros(Tend, num_gen);
    ...
    energy_revs_dp = zeros(Tend, num_gen)
    reg_revs_dp = zeros(Tend, num_gen)
    rsv_revs_dp = zeros(Tend, num_gen)
    var_om_costs_dp = zeros(Tend, num_gen)
    fuel_costs_dp = zeros(Tend, num_gen)
    start_costs_dp = zeros(Tend, num_gen)
    charge_costs_dp = zeros(Tend, num_gen)

    # per zone components - WELFARE 
    nse_cost = zeros(Tend, 1)
    unmet_rsv_cost = zeros(Tend, 1)
    ...
    for r in R
        ...
        if r == 1
            ...
            @constraint(EP, cSTOR_MaxRegChargeLink[y in STOR_REG_RSV, t in START_SUBPERIODS, w=1:W], EP[:vCHARGE][y,t,w]+EP[:vREG_charge][y,t,w] <= 
                EP[:eTotalCapEnergy][y]-s_dp[r-1,y])
        end
        ...
        if r == 1
            ...
            @constraint(EP, cSTOR_MaxRegRsvLink[y in STOR_REG_RSV, t in START_SUBPERIODS, w=1:W], EP[:vP][y,t,w]+EP[:vREG_discharge][y,t,w] +
                EP[:vRSV_discharge][y,t,w] <= s_dp[r-1,y] * efficiency_down(gen[y]))
        end
        ...
        if r == 1 ### XXX This is where the wrap-around or the warm-start needs to be implemented
            ...
            @constraint(EP, cSTOR_SOCLink[y in STOR_ALL, t in START_SUBPERIODS, w = 1:W], EP[:vS][y,t,w] ==
                s_dp[r-1,y]-(1 / efficiency_down(gen[y]) * EP[:vP][y,t,w])
                +(efficiency_up(gen[y]) *EP[:vCHARGE][y,t,w])-(self_discharge(gen[y]) * s_dp[r-1,y]))
        end
        ...
        @constraint(EP, cTC_MinDownTime_Link[y in THERM_COMMIT, t in 1:(Down_Time[y]-1), w = 1:W], 
            ...
            + sum(shut_dp[h,y] for h in lkad_hoursbefore(Tend,r+t-1,1:Down_Time[y]) if h < r) 
        ...
        @constraint(EP, cTC_MinUpTime_Link[y in THERM_COMMIT, t in 1:(Up_Time[y]-1), w = 1:W], 
            ...
            + sum(start_dp[h,y] for h in lkad_hoursbefore(Tend, r+t-1, 1:Up_Time[y]) if h < r)
        ...
        if r == 1 ### XXX This is where the wrap-around or the warm-start needs to be implemented
            ...
            @constraint(EP, cTC_CommitUnitsLink[y in THERM_COMMIT, t in START_SUBPERIODS, w=1:W], 
                EP[:vCOMMIT][y,t,w] == commit_dp[r-1,y] + EP[:vSTART][y,t,w] - EP[:vSHUT][y,t,w])
        end
        ...
        if r == 1 ### XXX This is where the wrap-around or the warm-start needs to be implemented
            ...
            @constraint(EP,cTC_MaxRampDownLink[y in THERM_COMMIT, t in START_SUBPERIODS,w=1:W],
            pgen_dp[r-1,y] - EP[:vP][y,t,w] <= ramp_down_fraction(gen[y]) * cap_size(gen[y]) *(EP[:vCOMMIT][y,t,w]-EP[:vSTART][y,t,w])
            - min_power(gen[y]) * cap_size(gen[y]) *EP[:vSTART][y,t,w]
            + min(rhinputs["pP_Max"][w][y,t],max(min_power(gen[y]), ramp_down_fraction(gen[y]) )) 
            * cap_size(gen[y]) *EP[:vSHUT][y,t,w])
        end
        ...
        if r == 1 ### XXX This is where the wrap-around or the warm-start needs to be implemented
            ...
            * cap_size(gen[y]) * EP[:vSTART][y,t,w]
            ...
            @constraint(EP,cTC_MaxRampUpLink[y in THERM_COMMIT, t in START_SUBPERIODS,w=1:W],
            EP[:vP][y,t,w]- pgen_dp[r-1,y] <= ramp_up_fraction(gen[y]) * cap_size(gen[y]) * (EP[:vCOMMIT][y,t,w]-EP[:vSTART][y,t,w])
            + min(rhinputs["pP_Max"][w][y,t],max(min_power(gen[y]) , ramp_up_fraction(gen[y]) )) 
            * cap_size(gen[y]) * EP[:vSTART][y,t,w]
            - min_power(gen[y]) * cap_size(gen[y]) *EP[:vSHUT][y,t,w])
        end
    ...
    writedlm(results_folder * "/revenue_reg_revs_dp.csv", reg_revs_dp', ',')
    writedlm(results_folder * "/revenue_rsv_revs_dp.csv", rsv_revs_dp', ',')
    writedlm(results_folder * "/revenue_var_om_costs_dp.csv", var_om_costs_dp', ',')
    writedlm(results_folder * "/revenue_fuel_costs_dp.csv", fuel_costs_dp', ',')
    writedlm(results_folder * "/revenue_start_costs_dp.csv", start_costs_dp', ',')
    writedlm(results_folder * "/revenue_charge_costs_dp.csv", charge_costs_dp', ',')
